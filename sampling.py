...
def get_sampling_fn(config, sde, shape, inverse_scaler, eps, radon):
  ...
  # Predictor-Corrector sampling. Predictor-only and Corrector-only samplers are special cases.
    ...
    sampling_fn = get_pc_sampler(radon=radon,
                                 sde=sde,
                                 shape=shape,
                                 predictor=predictor,
                                 corrector=corrector,
...
class ReverseDiffusionPredictor(Predictor):
  def __init__(self, sde, score_fn, probability_flow=False,vector=None):
    super().__init__(sde, score_fn, probability_flow)
    self.vector=vector

  def update_fn(self, x, t):
    f, G = self.rsde.discretize(x, t,self.vector)
    z = torch.randn_like(x)
    x_mean = x - f
...
class LangevinCorrector(Corrector):
  def __init__(self, sde, score_fn, snr, n_steps):
    ...
    self.radon = CT(img_width=self.size, radon_view=self.Nview, circle=False)
  ...
  def update_fn(self, x, t):
    ...
    for i in range(n_steps):
      data = x.reshape(1, 1,256, 256)
      vector = self.radon.degradation_fn_fbp(data)
      grad = score_fn(x,vector, t)
      noise = torch.randn_like(x)
...
def shared_predictor_update_fn(x, t, sde, model, predictor, probability_flow, continuous,radon, vector):
  """A wrapper that configures and returns the update function of predictors."""

  data = x.reshape(1, 1,256, 256)
  vector = radon.degradation_fn_fbp(data)
  score_fn = mutils.get_score_fn(sde, model,vector, train=False, continuous=continuous)
  if predictor is None:
    # Corrector-only sampler
    predictor_obj = NonePredictor(sde, score_fn, probability_flow)
  else:
    predictor_obj = predictor(sde, score_fn, probability_flow,vector)
  return predictor_obj.update_fn(x, t)


def shared_corrector_update_fn(x, t, sde, model, corrector, continuous, snr, n_steps, radon,cs=False,
...
def get_pc_sampler(radon, sde, shape, predictor, corrector, inverse_scaler, snr,
