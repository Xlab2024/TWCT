...
class DualInputCNN1(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        x = self.conv3(x)
        return x
class DualInputCNN2(nn.Module):
    def __init__(self):
        super(DualInputCNN2, self).__init__()
        self.conv1 = nn.Conv2d(10, 32, kernel_size=3, padding=1)
        self.conv2 = nn.Conv2d(10, 32, kernel_size=3, padding=1)
        self.conv3 = nn.Conv2d(10, 32, kernel_size=3, padding=1)
        # 使用一个卷积层将通道数从96直接降低到10
        self.conv4 = nn.Conv2d(96, 10, kernel_size=3, padding=1)
    ...
    def forward(self, x1, x2, x3):
        ...
        x = self.conv4(x)  # 直接降低通道数到10
        return x
class DualInputCNN3(nn.Module):
    def __init__(self):
        super(DualInputCNN3, self).__init__()
        self.conv1 = nn.Conv2d(10, 32, kernel_size=3, padding=1)
        self.conv2 = nn.Conv2d(10, 32, kernel_size=3, padding=1)
        # 使用一个卷积层将通道数从64直接降低到10
        self.conv3 = nn.Conv2d(64, 10, kernel_size=3, padding=1)
    ...
    def forward(self, x1, x2):
        ...
        x = self.conv3(x)
        return x
class DualInputCNN4(nn.Module):
    def __init__(self):
        super(DualInputCNN4, self).__init__()
        self.conv1 = nn.Conv2d(10, 32, kernel_size=3, padding=1)
        self.conv2 = nn.Conv2d(10, 32, kernel_size=3, padding=1)
        self.conv3 = nn.Conv2d(10, 32, kernel_size=3, padding=1)
        # 使用一个卷积层将通道数从96直接降低到10
...
def get_pc_radon_ADMM_TV(sde, predictor, corrector, inverse_scaler, snr,
    ...
    radon3 = CT(img_width=256, radon_view=2, circle=False)
    vector = torch.rand(1, 1, 256, 256)
...
def get_pc_radon_ADMM_TV_vol(sde, predictor, corrector, inverse_scaler, snr,
    ...
    def A_cg(x,number):
        ...
        # x_numpy = x_cpu.numpy()
        #
        # t1 = (_AT(_A(x)) + rho * _DzT(_Dz(x))).cpu().squeeze().numpy()
        #
        # t4 = x.cpu().squeeze().numpy()
        # return _AT(_A(x)) + rho * _DzT(_Dz(x))
        #return _AT(_A(x)) +  0.00000001*rho * _DzT(_Dz(x))+ 0.0473* number**0.5* _DxT(_Dx(x)) +0.0473* number**0.5* _DyT(_Dy(x))
        # return _AT(_A(x))
        # start_time = time.time()
        # end_time = time.time()
        # print("Total time2 taken: {:.8f} seconds".format(end_time - start_time))
        return _AT(_A(x))+ 0.2* number**0.5* _DxT(_Dx(x)) +0.2* number**0.5* _DyT(_Dy(x))
    ...
    def CS_routine(x, ATy, niter=20,number=None):
        ...
        for i in range(niter):
            ...
            A_cg_with_number = partial(A_cg, number=number)
            #b_cg = ATy + 0.00000001* rho * (_DzT(del_z) - _DzT(udel_z))+ 0.0473* number**0.5*(_DxT(del_x) - _DxT(udel_x))+0.0473* number**0.5*(_DyT(del_y) - _DyT(udel_y))

            b_cg = ATy+ 0.2* number**0.5*(_DxT(del_x) - _DxT(udel_x))+0.2* number**0.5*(_DyT(del_y) - _DyT(udel_y))

            # b_cg = ATy
            # b_cg = ATy + rho * (_DzT(del_z) - _DzT(udel_z))
            x = CG(A_cg_with_number, b_cg, x, n_inner=1)
            lamb_12=2*0.00025* number**0.5

            del_x = shrink(_Dx(x) + udel_x, lamb_12/  rho)
            del_y = shrink(_Dy(x) + udel_y, lamb_12 / rho)
            del_z = shrink(_Dz(x) + udel_z, lamb_1 / rho)
    ...
    #     return radon_update_fn
    def  get_ADMM_TV_fn():
        def ADMM_TV_fn(x, measurement=None,number=None):
            with torch.no_grad():
                start_time = time.time()
                ATy = _AT(measurement)
                end_time = time.time()
                print("Total time2 taken: {:.8f} seconds".format(end_time - start_time))
                x, x_mean = CS_routine(x, ATy, niter=10,number=number)
    ...
    def pc_radon(model, data, measurement=None, fbp=None):
        with torch.no_grad():
            x = sde.prior_sampling(data.shape).to(data.device)
            ...
            loop_start_time = time.time()  # Start time of the entire loop
            ...
            for i in tqdm(range(sde.N)):
                ...
                total_agg_time += agg_end_time - agg_start_time
                ...
                x, x_mean = mc_update_fn(x, measurement=measurement, number=i)
                admm_end_time = time.time()
                total_admm_time += admm_end_time - admm_start_time

                if save_progress:
                    if (i % 50) == 0:
                        print(f'iter: {i}/{sde.N}')
                        plt.imsave(save_root / 'recon' / 'progress' / f'progress{i}.png', clear(x_mean[0:1]),
                                   cmap='gray')

            loop_end_time = time.time()  # End time of the entire loop
            print(f'Total loop time: {loop_end_time - loop_start_time:.4f}s')

            avg_agg_time = total_agg_time / sde.N
            avg_admm_time = total_admm_time / sde.N
            print(f'Average Aggregation time: {avg_agg_time:.4f}s')
            print(f'Average ADMM TV time: {avg_admm_time:.4f}s')
