...
def get_sde_loss_fn(sde, train, reduce_mean=True, continuous=True, likelihood_weighting=True, eps=1e-5):
  ...
  def loss_fn(model, batch,vector):
    ...
    """
    score_fn = mutils.get_score_fn(sde, model,vector, train=train, continuous=continuous)
    t = torch.rand(batch.shape[0], device=batch.device) * (sde.T - eps) + eps
    z = torch.randn_like(batch)
    mean, std = sde.marginal_prob(batch, t)
    perturbed_data = mean + std[:, None, None, None] * z
    score = score_fn(perturbed_data,vector, t)
...
def get_step_fn(sde, train, optimize_fn=None, reduce_mean=False, continuous=True, likelihood_weighting=False):
  ...
  def step_fn(state, batch, vector):
    ...
    if train:
      ...
      optimizer.zero_grad()
      loss = loss_fn(model, batch, vector)
      loss.backward()
