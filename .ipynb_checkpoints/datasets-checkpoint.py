...
class AAPM(Dataset):
  ...
  def get_vector(self, data):
    ...
    image4d = torch.from_numpy(data)
    vector = self.radon.degradation_fn_fbp(image4d)
    ...
    # TomoW = 256
    #
    # vol_geom = astra.create_vol_geom(TomoW, TomoW)  # 创建的体素尺寸都为1,所以需要和物理模型做比例转换
    # # proj_geom = astra.create_proj_geom('parallel', 1.0, 384, np.linspace(0,np.pi,2,False))
    # image = data.squeeze()
    # # 从真实物理模型到模拟投影环境的比例映射
    # ratios = TomoW / ObjSize
    # dU = CCDsize * ratios
    # src = SA * ratios
    # det = (SD - SA) * ratios
    #
    # proj_geom = astra.create_proj_geom('fanflat', dU, ProW, np.linspace(0, np.pi, 2, False), src, det)
    ...
    #
    # vol_geom_rec = astra.create_vol_geom(256, 256)  # 创建的体素尺寸都为1,所以需要和物理模型做比例转换
    # # Create a data object for the reconstruction
    # rec_id = astra.data2d.create('-vol', vol_geom_rec)
    #
    ...
    # vector = torch.from_numpy(rec)
    # vector = vector.unsqueeze(0)  # 在第一个维度上添加尺寸为1的维度
    # vector = vector.unsqueeze(0)  # 在第二个维度上添加尺寸为1的维度
    return vector  # VECTOR_SIZE 是向量的大小
