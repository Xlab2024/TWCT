...
def train(config, workdir):
  ...
  radon = CT(img_width=256, radon_view=2, circle=False)
  ...
  if config.training.snapshot_sampling:
    ...
    sampling_fn = sampling.get_sampling_fn(config, sde, sampling_shape, inverse_scaler, sampling_eps,radon)
  ...
  for epoch in range(1, config.training.epochs):
    ...
    for step, (batch, vector) in enumerate(train_dl, start=1):
      batch = scaler(batch.to(config.device))
      vector = vector.to(config.device)

      # (b, 1, 320, 320, 2) --> (b, 2, 320, 320)
      # batch = kspace_to_nchw(torch.view_as_real(batch))
      # Execute one training step
      loss = train_step_fn(state, batch,vector)
